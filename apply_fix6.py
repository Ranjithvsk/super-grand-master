...
# The exact anchor in the file (two newlines after btn-out:hover rule)
target = "#btn-out:hover{background:#302e2b;color:#fff}\n\n</style>"
...
if target in src:
    ...
    print("ERROR: anchor still not found - check whitespace")
    idx = src.find("#btn-out:hover")
    print("Exact chars after rule:", repr(src[idx+46:idx+80]))

with open('index.html', 'w', encoding='utf-8') as f:
    f.write(src)

print("Done. Run: git add . && git commit -m 'Fix profile CSS' && git push origin HEAD:main")
